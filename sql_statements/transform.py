...
dim_call_resolution_status = '''INSERT INTO staging.dim_call_resolution_status (
    ...
    SELECT
        cl.agent_id,
        COUNT(cl.agent_id) OVER (PARTITION BY cl.agent_id ORDER BY cl.agent_id DESC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS calls_received,
        SUM(CASE WHEN cl.status = 'Resolved' THEN 1 ELSE 0 END) OVER (PARTITION BY cl.agent_id ORDER BY cl.agent_id DESC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS calls_resolved
    FROM raw_data.call_log cl;
    '''
...
ft_call_transactions = '''INSERT INTO staging.ft_call_transactions (
    ...
    SELECT
        ...
        rtr.resolution_duration_in_hours,
        rcd.call_duration_in_seconds
    FROM staging.dim_call_log cl
    JOIN staging.dim_call_details cd ON cl.call_log_id = cd.call_id
    JOIN staging.dim_call_duration cdu ON cl.agent_id = cdu.agent_id
    ...
    JOIN raw_data.call_details rcd USING (call_id)
    ;'''

transformation_queries = ['dim_call_log', 'dim_call_details', 'dim_call_duration', 'dim_call_resolution_status', 'dim_resolution_time_rank', 'ft_call_transactions']
